...
- **Daily meals**: Berto eats one lunch, one snack and one dinner a day, and the app fills all three in automatically on the morning tick (`lib/meal-suggest.ts` — it reads his staples, his food principles, and what he's eaten lately, then generates a photo per sitting). You don't need to do this. Use `set_daily_meal` with a `slot` only when he asks for a specific dish or wants one changed, and call `list_meal_history` first for recent picks and thumbs up/down. He gives feedback on the Nutrition and Home screens, not in chat.

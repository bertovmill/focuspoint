...
// Shared dream generation logic used by both the cron (GET) and manual trigger (POST).
async function runDream() {
  ...
  if (thoughts.length === 0 && todos.length === 0) {
    return { message: "No data to dream on yet — add some notes or tasks first." };
  }
...
export async function GET(req: NextRequest) {
  ...
  try {
    return NextResponse.json(await runDream());
  } catch (err) {
...
export async function POST() {
  try {
    ...
    console.error("Dream manual trigger failed:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Unknown error" },
      { status: 500 }
    );

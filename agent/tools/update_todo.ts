...
export default defineTool({
  description:
    "Edit an existing todo's title, priority, due date, or recurrence. Use list_todos first if you don't know its id.",
  ...
  async execute({ id, ...patch }) {
    ...
    const [row] = await sql`
      ...
      WHERE id = ${id}
      RETURNING id, title, priority, due_date, recurrence
    `;

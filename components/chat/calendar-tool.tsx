...
/**
 * In-chat calendar card for Cael's `list_calendar_events` tool. Ported from the
 * assistant-ui tool UI: takes the tool part's lifecycle state and output
 * directly, so it renders from eve's message projection with no runtime layer.
 */
...
export function CalendarTool({
  ...
  running: boolean;
  result: CalendarResult | undefined;
}) {
  if (running || result === undefined) {
    return (
      <CalendarCard>

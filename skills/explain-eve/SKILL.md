---
name: explain-eve
description: Use when someone asks what eve is, what eve agents can do, how the eve framework works, or how to build an eve agent. Loads an in-depth, accurate primer on Vercel's eve agent framework so you can explain its capabilities and scaffold a first agent.
---

# Explaining the eve framework

Use this when someone asks "what is eve?", "what can eve do?", "how does an eve
agent work?", or "how would I build an eve agent?". Match the depth of your
answer to the question: a casual "what is eve?" gets the one-liner plus a couple
...
home on the filesystem, and eve discovers that structure automatically.
...
move or rename it and its identity moves with it. Start with only
`instructions.md` (and optionally `agent.ts`) and add the other folders as the
agent grows.
...
- **Load skills on demand.** Longer procedures live as markdown and get pulled
  ...
  disclosure, so the agent stays focused and cheap.
...
- **Run on a schedule.** Cron-style schedules let the agent act proactively
  (e.g. a weekly recap or a morning digest).
...
- **Stay durable across turns.** A session is more than one request/response —
  ...
  SDK; eve handles the machinery so tools focus on the work.
...
| `agent/skills/*` | Procedures loaded only when useful |
